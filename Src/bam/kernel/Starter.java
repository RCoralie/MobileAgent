...
/** Permettant de configurer les caractéristiques d’un serveur via un fichier xml de description
 	> Parse le XML : 
 		> Crée un serveur
...
public class Starter{
	...
	protected bam.kernel._Server server; /** le server associé à ce starter */
	protected BAMServerClassLoader loader; /** le Loader utilisé */
	protected Class<bam.kernel.Server> classe; /** la classe du server : bam.kernel.Server */
	...
	public Starter(String... args){
		...
		try {
			...
			String loggerName = "bam/"+InetAddress.getLocalHost().getHostName()+"/"+args[1];
...
//			logger.setUseParentHandlers(false);
	...
	protected void createServer(int port, String name) throws MalformedURLException, ClassNotFoundException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		...
		classe = (Class<bam.kernel.Server>)Class.forName("bam.kernel.Server",true,loader);
